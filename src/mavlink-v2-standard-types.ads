...
package MAVLink.V2.Standard.Types is
   ...
   type Firmware_Version_Type is new Interfaces.Unsigned_8;
   --  These values define the type of firmware release. These values indicate
   --  the first version or release of this type. For example the first alpha
   --  release would be 64, the second would be 65.

   function Dev return Firmware_Version_Type is (0)
     with Static;
   --  development release

   function Alpha return Firmware_Version_Type is (64)
     with Static;
   --  alpha release

   function Beta return Firmware_Version_Type is (128)
     with Static;
   --  beta release

   function Rc return Firmware_Version_Type is (192)
     with Static;
   --  release candidate

   function Official return Firmware_Version_Type is (255)
     with Static;
   --  official stable release

   subtype Firmware_Version_Type_Well_Known is Firmware_Version_Type
     with Static_Predicate => Firmware_Version_Type_Well_Known in
       Dev
       | Alpha
       | Beta
       | Rc
   ...
   function Well_Known_Image
     (Value : Firmware_Version_Type_Well_Known) return String is
       (case Value is
        when Dev => "Dev",
        when Alpha => "Alpha",
        when Beta => "Beta",
   ...
   function Image (Value : Firmware_Version_Type) return String is
     (if Value in Firmware_Version_Type_Well_Known
      then Well_Known_Image (Value) else "Unknown:" & Value'Image);

end MAVLink.V2.Standard.Types;
